...
public abstract class BatchingStrategy {
   ...
   */
    public List<JUnitHost.TestInfo[]> getTestBlocks(final String syntheticModuleName) {
        final Set<JUnitHost.TestInfo> allTestsInModule = this.getTestsForModule(syntheticModuleName);
        final List<JUnitHost.TestInfo[]> testBlocks = new ArrayList<JUnitHost.TestInfo[]>();
        if (allTestsInModule.size() > 0) {
            final JUnitHost.TestInfo[] testBlock = allTestsInModule.toArray(new JUnitHost.TestInfo[allTestsInModule.size()]);
