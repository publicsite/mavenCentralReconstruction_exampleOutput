...
public class StandardConnectionPoolDataSource
	 ...
	 */
    public synchronized void setLockModeToWait(final int seconds) throws SQLException {
        if (this.lockModeWait != seconds) {
            if (seconds > 0) {
                this.execute("SET LOCK MODE TO WAIT " + seconds);
            }
            else if (seconds == 0) {
                this.execute("SET LOCK MODE TO NOT WAIT ");
            }
            else {
                this.execute("SET LOCK MODE TO WAIT ");
            }
            this.lockModeWait = seconds;
        }
    }
